...
export default function App() {
  ...
  const hideDialog = (updatedCustomer) => {
    ...
    setCustomers((current) => {
      const index = current.findIndex((obj => obj["uid"] === dialog["customer"]["uid"]));
      current[index] = {
    ...
    db.transaction((tx) => {
      tx.executeSql('UPDATE customers SET name=(?) WHERE uid=(?)', [dialog["customer"]["name"], dialog["customer"]["uid"]]);
    });
  };
  ...
  const deleteCustomer = async (customer) => {
    ...
    // 1. Set the new local customer state
    setCustomers((current) => {
      return current.filter(obj => obj["uid"] !== customer["uid"])
    })
